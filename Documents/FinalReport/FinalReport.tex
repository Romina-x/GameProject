...
The Strategy pattern is a design pattern used to define a family of behaviors and allows them to be interchangeable. In game development, this pattern is often used to handle different behaviors that an object can perform, such as various movement styles for enemies, different attack strategies for characters, or different types of player abilities. The way this pattern works is by defining a set of behaviors in separate classes, and let the client class decide which behavior to use at runtime.
\newline
...
For other UI screens, the same principles of visual aesthetics and coherence with the game’s aesthetic are important, just as much as for level design. A well designed user interface should be ergonomic, being intuitive and easy to navigate using the given input controls. In game development, this is typically achieved through the use of wireframes, which are storyboards mapping out user interaction flows \cite{Rogers2014}. These wireframes include drawings that depict screen transitions, button interactions, item pickups, and more, and are later transformed into functional prototypes that are playtested to refine the usability and appeal of the interface.\newline
...
\section{User Experience}\ref{UX}
...
The current font size is \the\fontdimen2\font.
...
To test whether these optimisations have helped the game run smoothly, this was tested through both the Unity Profiler and beta testing. The Unity Profiler provides realtime performance information about your application, giving an overview of resource allocation while developing the application \cite{UnityProfiler}. This was used on the device that was used to develop the game, which is a laptop with an Intel Core i5-12500H (2.50 GHz) processor and 16GB of RAM, running Windows 11. The profiler measured these statistics on a simple playthrough of the game, which involved starting from the title screen, navigating to the first level, and playing it to the end. The results for CPU usage are displayed in figure \ref{fig:label_CPU}.
\begin{figure}[H]
    \centering
...
Briefly introduce why testing was important to your development process. Mention that multiple types of testing were used to ensure functionality, usability, and performance.

Example:
To ensure the game was functional, user-friendly, and performant, various testing approaches were used throughout development. These included automated testing for code reliability, playtesting for UX evaluation, beta testing for broader feedback, and performance testing to verify system requirements.

2. Unit Testing

Brief Explanation:
Define unit testing and its role in ensuring individual components of the code behave correctly.

What You Did:
Explain what systems you wrote unit tests for (e.g., health system, score system), what tools you used (e.g., Unity Test Framework), and how effective it was.

Example:
Unit testing involves testing individual code units in isolation to verify they perform as expected. In this project, unit tests were written using Unity’s built-in Test Framework to validate systems like the player health, damage handling, and scoring. These tests were helpful during refactoring, ensuring changes didn’t break core gameplay logic.

3. Playtesting

Brief Explanation:
Define playtesting as observing users interact with the game to identify usability issues and gather design feedback.

What You Did:
Mention when in development you did this, who tested it (e.g., peers, friends), and what changes you made based on their feedback.

Example:
Playtesting was used early and mid-way through development to assess how intuitive and enjoyable the gameplay was. Informal sessions with classmates helped refine movement controls, attack feedback, and level design pacing. Based on their feedback, adjustments were made to tutorial prompts and hit detection.

4. Beta Testing

Brief Explanation:
Define beta testing as gathering structured feedback from external users near the end of development.

What You Did:
Explain how you distributed the build, how many users tested it, what feedback you collected (performance, usability), and how you responded to it.

Example:
A formal round of beta testing was conducted near the end of development by distributing the game build to seven users with varying PC setups. They were asked to play through the game and complete a short survey. Their feedback confirmed smooth performance across devices and highlighted potential feature improvements like a goal indicator and additional weapon types, which were noted for future development.

5. Performance Testing

Brief Explanation:
Define performance testing as assessing how efficiently the game runs under different conditions (frame rate, memory usage).

What You Did:
Mention using the Unity Profiler, what you tested (CPU, GPU, memory, rendering), and what conclusions you drew.

Example:
The Unity Profiler was used to assess CPU and memory usage during gameplay. Tests showed consistent frame rates below the 60 FPS threshold on the development device, with minor spikes during scene loads or combat. This indicated the game was running efficiently under normal conditions. However, to validate performance more broadly, beta testers were asked to report how well the game ran on their systems, which yielded positive feedback.
